...
const dict = {
  ...
  "dash.backup.hint": {
    ...
    fr: "Téléchargez une copie complète des données de votre entreprise (employés, présences, jours fériés personnalisés et congés) sous forme de fichier JSON. Conservez-le en lieu sûr (Google Drive, disque externe) pour toute éventualité. Vous pouvez le faire aussi souvent que vous le souhaitez — cela n'affecte pas les données dans ZyntrAbsen.",
  },
  ...
  "dash.backup.done": { es: "Listo — respaldo descargado con {emp} empleados y {att} marcas de asistencia.", en: "Done — backup downloaded with {emp} employees and {att} attendance records.", id: "Selesai — cadangan berhasil diunduh dengan {emp} karyawan dan {att} catatan kehadiran.", fr: "Terminé — sauvegarde téléchargée avec {emp} employés et {att} pointages." },
  ...
  "dash.backup.restoreDone":       { es: "✓ Restauración completa — {emp} empleados y {att} registros de asistencia procesados.", en: "✓ Restore complete — {emp} employees and {att} attendance records processed.", id: "✓ Pemulihan selesai — {emp} karyawan dan {att} catatan kehadiran diproses.", fr: "✓ Restauration complète — {emp} employés et {att} enregistrements de présence traités." },
